...
任務在執行前，必須從以下 6 個面向逐一思考，有風險的面向須明確指出並標註嚴重等級（🔴 重大 / 🟡 中等 / 🟢 輕微）。

本文件中標 `（R-NNN）` 的規則源自真實失敗——改或刪該規則前，先讀 retro/RETRO.md 對應條目確認變更不會重開該失敗。

**只寫觸及的面向**：與本任務無關的面向不逐節填寫，改在報告開頭以一行 `不適用：<面向清單>` 帶過（例：`不適用：安全、資料、效能`）。**面向未寫不等於已評估為無風險——判為不適用者仍須列名，漏列即視同未做**（六個面向必須全部出現在「有風險節」或「不適用」行之一，缺一即報告不完整）。
...
## 3. 資料風險 (Data Risk)
- 是否涉及 DB schema 變更（migration）；是否符合 .agent-flow/ROUTER.md 的資料庫規則
- 是否有資料遺失、截斷、型別不相容的風險
...
- **無關的面向不逐節填寫**，列進報告開頭的 `不適用：<面向清單>` 一行即可——六面向必須全部出現在「有風險節」或「不適用」行之一，漏列即視同未做
- **各等級的流程後果**（2026-09-22 起內聯於此）：🔴 → 擋住後續步驟，須先修改 Spec（補前置條件／縮小範圍／釐清描述）再重新分析，直到無 🔴；🟡 → 記錄於報告並帶入對應 task item 的備註；🟢 → 提醒即可，不擋
  - 原本此三級後果住在 eval-flow 前置 1 的判斷規則，該前置步驟已於 2026-09-22 刪除（Tier 2 流程瘦身，run `2026-09-22-tier2-slimming`），故改為內聯。本 skill 現在的唯一呼叫路徑是 **hotfix 補債**（見 `.agents/skills/eval-flow/references/rare-paths.md` 的欠帳規則）
