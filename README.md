...
關鍵 gate 不只靠文字約束，由 PreToolUse hook（`.claude/hooks/gate-check.sh` → `eval_gates.py`，matcher `Bash|Task|Agent`）硬性攔截。攔截點有二：`git commit`（gate 1–4）與 subagent 呼叫（gate 5–7）：
...
6. **欠帳 gate**：任一 manifest 的 `debt` 非空（hotfix 遺留）→ 擋新 run 的 subagent 呼叫，直到補完 risk 分析／回歸測試／retro
7. **單一 run gate**：本工作區存在其他 in_progress 的 manifest → 擋（一個 worktree 同時只跑一個 run；並行開 `git worktree`）
