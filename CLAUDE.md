...
| Tier | 進入條件（**全部**滿足） | 路徑 |
|---|---|---|
| **0 微調** | 無新行為（純樣式／文案／參數調整，無新邏輯／新分支；**bugfix 例外**：把實際行為修回預期行為、不新增行為面者，視同「無新行為」）＋ 不觸及高風險面（同 Tier 1 的排除清單，1 行也不例外）＋ 1~2 個檔案、每檔 ≤30 行（以 `git diff` 變更行數計，增＋刪）。**例外**：同一種機械式改動跨多檔重複套用（如同一句文案、同一個名稱替換），檔數不受 2 檔限制，但每檔仍 ≤50 行 | 直接改，不建任何檔 |
| **1 明確功能** | 需求無歧義（一句話講得清 DoD）＋ 單一使用路徑（單角色、無分支情境）＋ 預估 ≤2 tasks（合計 ≤8 items／各 ≤300 行）＋ **完全不觸及 auth／權限、金流／交易、DB schema 變更、部署／環境設定** | 載入 `eval-flow` skill，走「Tier 1 精簡路徑」 |
...
- **高風險面的邊界（機械判準）**：觸發排除的是**可執行邏輯／判定行為／設定值**的變更（gate 判斷式、權限規則、schema 定義、部署參數）；**純註解、訊息字串、文件說明、新增測試**不觸發高風險排除——判準看 diff 動到的行是不是可執行邏輯行，不是看檔案在哪個目錄（例：`.claude/hooks/*.py` 的 gate 判定式＝高風險；同檔的註解與錯誤訊息字串＝否）。此類變更仍照其他軸（歧義／大小）正常判級。Tier 2 是重型流程，入口收窄、保留給真正需要的
...
- scout 蒐證檔放在 `scout/` 資料夾：`scout/<run_id>.md`（前置 1.5 產出即存檔）
