...
- **高風險面的邊界（機械判準，2026-07-25 精確化）**：觸發排除的是**可執行邏輯／判定行為／設定值**的變更（gate 判斷式、權限規則、schema 定義、部署參數）；**純註解、訊息字串、文件說明、新增測試**不觸發高風險排除——判準看 diff 動到的行是不是可執行邏輯行，不是看檔案在哪個目錄（例：`.claude/hooks/*.py` 的 gate 判定式＝高風險；同檔的註解與錯誤訊息字串＝否）。此類變更仍照其他軸（歧義／大小）正常判級。Tier 2 是重型流程，入口收窄、保留給真正需要的
...
- **升級逃生門**：Tier 1 執行中若發生下列任一 → **中止，升級 Tier 2**，補跑缺的前置（產 Spec、跑 usage、正式風險分析）：
  - 分拆後超過 2 tasks 或合計超過 8 items（硬上限）
  - 出現預估遠超 300 行且無法在合計 ≤8 items 內合理消化的 item（在 Tier 1 情境下，代表這功能其實不小，應改走完整流程；非要求硬拆）
