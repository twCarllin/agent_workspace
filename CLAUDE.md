...
| Tier | 進入條件（**全部**滿足） | 路徑 |
|---|---|---|
| **0 微調** | 無新行為（純樣式／文案／參數調整，無新邏輯／新分支；**bugfix 例外**：把實際行為修回預期行為、不新增行為面者，視同「無新行為」）＋ 不觸及高風險面（同 Tier 1 的排除清單，1 行也不例外）＋ ≤3 個檔案、合計 ≤80 行（以 `git diff` 變更行數計，增＋刪）。**例外**：同一種機械式改動跨多檔重複套用（如同一句文案、同一個名稱替換），檔數不受 3 檔限制，但每檔 ≤50 行 | 直接改，不建任何檔 |
| **1 明確功能** | 需求無歧義（一句話講得清 DoD）＋ 單一使用路徑（單角色、無分支情境）＋ 預估 ≤2 tasks（合計 ≤8 items／各 ≤300 行）＋ **不變更 auth／權限、金流／交易、DB schema、部署／環境設定的本體**（僅呼叫／經過既有機制＝使用，不觸發；判準見防濫用規則「高風險面的邊界」） | 載入 `eval-flow` skill，走「Tier 1 精簡路徑」 |
| **2 完整功能** | 以上任一不成立：有歧義 OR 偏大 OR 多角色／多情境 OR 變更上述任一高風險面本體 | 載入 `eval-flow` skill，走完整「Tier 2 路徑」 |
...
- **高風險面的邊界（機械判準）**：觸發排除的是**可執行邏輯／判定行為／設定值**的變更（gate 判斷式、權限規則、schema 定義、部署參數）；**純註解、訊息字串、文件說明、新增測試**不觸發高風險排除——判準看 diff 動到的行是不是可執行邏輯行，不是看檔案在哪個目錄（例：`.claude/hooks/*.py` 的 gate 判定式＝高風險；同檔的註解與錯誤訊息字串＝否）。**第二道判準（變更本體 vs 使用既有機制）**：排除僅在 diff 修改高風險機制**自身**時觸發——auth／權限的判定邏輯、金流計算／交易處理邏輯、DB schema 定義、部署設定內容；呼叫既有權限 helper、在既有 auth 保護路徑下新增功能、讀取既有設定＝**使用**，不觸發（例：新頁面掛在既有 login guard 之後＝否；改 guard 的判斷式＝是）。兩道判準**同時成立**（動到可執行邏輯行＋動到高風險機制本體）才觸發排除。此類變更仍照其他軸（歧義／大小）正常判級。Tier 2 是重型流程，入口收窄、保留給真正需要的
...
- **升級逃生門**：Tier 1 執行中若發生下列任一 → **中止，升級 Tier 2**，補跑缺的前置（產 Spec、跑 usage、正式風險分析）：
  - 出現預估遠超 300 行（在 Tier 1 情境下，代表這功能其實不小，應改走完整流程；非要求硬拆）
