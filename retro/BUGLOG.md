...
每筆 bugfix 修完由主 flow append 一行；同模組或同根因分類第 2 次命中時提煉約束句升級進 RETRO.md（規則見 bug-diagnosis skill「retro 兩層制」節）。
...
- 2026-09-21［.claude/hooks/eval_state.py｜run_verify.py／append_event 事件留痕］驗證盲點：`append_event` 過濾 `command` 鍵（argparse 子命令 dest 同名），而 `add-verification --command` 與 run_verify 的 `Namespace(command=...)` 都用該鍵傳指令原文，事件靜默只剩 `exit_code`；既有測試只斷言 `args["exit_code"]`、未逐鍵斷言留痕內容，2026-08-03 上線至今無人察覺，直到要統計全套次數才發現無資料可用。同分類（驗證盲點）第 3 次命中、前兩條已 ↑RETRO（R-005 端到端契約）；本條屬「旁路留痕欄位靜默丟失」，R-004／R-005 皆未涵蓋，是否加嚴既有條目交使用者裁決
