---
name: eval-scoring
description: eval-scorer 的評分基準：五維度（Clarity、Completeness、Testability、Non-functional、Technical_constraints）各 0–2 分，quality_score = 五維度加總（0–10），扣分原因規則與不變量、自我校驗問題、輸出格式。觸發語：eval-scorer subagent 執行評分時載入。不適用於：code-reviewer 的問題審查（走 review-checklist）、任務描述審查（走 task-checklist）。
---

# Eval Scoring（評分基準）

> 本 skill 由 **`eval-scorer` subagent** 載入使用（Eval Flow 循環 step 6）。評分結果 append 進 `eval_state.json` 對應 sub_task 的 `rounds`，格式與不變量受 hook（`eval_gates.py`）強制驗證。
