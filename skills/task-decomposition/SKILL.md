...
- **禁不可驗評價詞（驗收語紀律）**：DoD 與契約 row 不得以評價詞作為驗收語——「正常」「完善」「正確」「妥善」「良好」「穩定」「優化」「合理」等無機械判準的詞，除非同句緊跟判準（例「正常＝exit 0 且 stdout 含 X」）。評價詞是上條「可驗斷言」的高頻逃逸口：句子看似驗收條件，實際無法對照任何輸出判真偽
- **step5 憑據記號**：DoD 條目的憑據本質上只能由循環 step 5 實跑產生（視覺呈現、實機行為——step 3 審查時點必然無憑據可核）時，條目尾標 `[憑據:step5]`。記號定義住本 skill（單一枚舉點）；消費規則住 task-verifier 定義（checker 記 🔍 step 5 待驗、不計升級）與 eval-flow 循環 step 5（主 flow 收口核對）。**僅限實跑類憑據**——可用測試斷言、grep、靜態檢查驗的條目不得標（標了等於把可自動驗的事推給人工，checker 對憑據的核對就被架空）
...
- DoD／契約 row 以評價詞作驗收語且無機械判準（「錯誤處理完善」「功能正常」——無法對照輸出判真偽的句子不是 DoD）
- 引入新行為的實作 item，DoD 沒綁該 item 的單元測試（測試被推遲到整合階段，新行為裸奔）
- 引入新行為的實作 item 無行為契約表，或契約表全是 happy path（無邊界輸入）、或寫的是實作細節而非可觀察效果
- item 有 ≥2 個可獨立取值輸入面（旗標×模式、參數×路徑、選項×子命令）卻無組合 row——只驗各輸入面單獨取值，未驗交叉組合下的可觀察效果
- 有意的行為變更卻無「作廢舊行為清單」或實作 item 未標測試同步段——舊測試更新散進 writer 的迭代迴圈，逐輪踩雷、貴 model 做機械替換
- item 把同句規範文字投放 ≥2 個異質語境卻未逐語境核對關鍵詞語義碰撞——投放後各語境對同一關鍵詞下相反或歧義指令（R-006 家族）
