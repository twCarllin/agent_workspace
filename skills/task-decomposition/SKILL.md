...
## 推理密度（演算法密集 item 須附偽代碼骨架）

行數之外的第二軸：**推導負擔**。機械判準：item 規格含 **≥3 步、步驟間有資料流依賴**的演算法程序（多步解析、座標聚類、狀態機）＝演算法密集。此類 item 只給散文步驟不合格——**Spec（Tier 2）或 manifest `spec_inline`（Tier 1；主 flow 直建 task 檔時同樣適用）須附偽代碼骨架**：函式切分、各函式輸入輸出、核心資料結構。item DoD 只指向骨架所在（實作內容不進 DoD，沿「禁止內嵌實作內容」規則）。

原則：**code-writer 是執行者，不是推導者**。「散文規格 → 程式結構」的推導若留到執行期，全部落在 writer 的 thinking——實測（2026-08-10 TW_Analysis）：五步驟純散文演算法規格使 code-writer 連續 4 次 thinking 燒滿 32k output token、零產出、agent 中止。推導成本要花在拆分期（產出可審的骨架），不花在執行期（燒滿即死、無殘留物）。
...
- **item 行數預算（校準值，實測後可調）**：每 item 全部內容（含 DoD 與契約表）≤ **12 行**——**計數以邏輯單元為準**：描述、每條 DoD、每條契約 row、情境各計 1，不論排版折疊成單行或逐行展開；DoD ≤ **5 條**、每條為一句可驗斷言（grep／指令／可觀察效果；排版單行分號串接或逐行皆可）。超標＝實作細節誤入 DoD 的訊號，瘦身或再拆 item（12＝描述 1＋DoD 上限 5＋契約表上限 5＋情境 1 的算術和；5＝合規 DoD 壓縮實測殘量 3–6 的上緣）。契約 row 需求超過 5 條時以**再拆 item** 消化，不得為湊預算砍邊界覆蓋（上方「至少 1 條邊界」為硬性）
- **禁止內嵌實作內容**：要寫進目標檔的替換全文、程式碼片段不得放進 DoD——精確措辭重要時，住 Spec（Tier 2）或 manifest `spec_inline` 補註（Tier 1），DoD 只留指向與斷言
- **自足性＝指向式引用**：「自足」是讀 task 檔＋其引用的 spec／usage／impact 檔即可接手，不是全部內嵌——情境以 id、風險與慣例以報告節名指向，不重述報告內容（task 檔 header 註記同理）
...
- 演算法密集 item（≥3 步、步驟間有資料流依賴）只附散文步驟、無偽代碼骨架——「規格 → 程式結構」的推導全落在 writer 執行期的 thinking，燒滿 output 上限即中止、四次重試零產出（2026-08-10 實證）
...
- DoD 內嵌整段替換文字／程式碼片段——實作內容住 Spec 或 `spec_inline`，DoD 只留指向與斷言（行數預算見輸出格式節）
- task 檔重述 Spec／usage／impact 報告已有的內容——自足性靠指向式引用達成，重述使 task 檔膨脹且兩處易漂移
