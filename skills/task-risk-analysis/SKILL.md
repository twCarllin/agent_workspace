...
- **各等級的流程後果**（2026-09-22 起內聯於此）：🔴 → 擋住後續步驟，須先修改 Spec（補前置條件／縮小範圍／釐清描述）再重新分析，直到無 🔴；🟡 → 記錄於報告並帶入對應 task item 的備註；🟢 → 提醒即可，不擋
  - 原本此三級後果住在 eval-flow 前置 1 的判斷規則，該前置步驟已於 2026-09-22 刪除（Tier 2 流程瘦身，run `2026-09-22-tier2-slimming`），故改為內聯。本 skill 現在的唯一呼叫路徑是 **hotfix 補債**（見 `skills/eval-flow/references/rare-paths.md` 的欠帳規則）
