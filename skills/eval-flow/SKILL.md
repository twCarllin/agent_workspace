...
> 本 skill 由主 flow 在 Router 判定 **Tier 1 或 Tier 2** 後載入執行。Router 分級表與防濫用規則住在 CLAUDE.md，不在此重述。Tier 2 走完整路徑（前置 0–1 ＋循環）；Tier 1 走文末「Tier 1 精簡路徑」（跳過部分前置，共用循環）。
>
...
4. 審查結果的處置：
   ...
   - **發現不得自我授權（scope 防線）**：任何發現（含 🟡 建議）要進 fixing，主 flow 必須先指名其**對映依據**——本 item 的 DoD 條目、契約 row、Spec／spec_inline 句、或既有硬規則（CLAUDE.md／skill 條文）之一，記入該輪處置摘要（隨收尾回報呈現）。
...
6. **收尾順序（**hook 強制**，見「Gate 的硬性執行」，完整清單見 `references/gates.md`）**：
   ...
   - ①將 `eval_state.json` 歸檔為 `run/<run_id>.eval.json`（保留審查記錄的永久紀錄），manifest 填 `status: "completed"`、`phase: "completed"`，**清除 `eval_state.json`**（熱 scratchpad，收尾即清；失敗收尾則保留現場）。審查落檔（`review-st*-r*.md`）與 `mine_log.json` 已於 2026-09-22 起不再產生（D1），收尾不再需要清除這兩類檔案
